...
var _1cc=node.name.toLowerCase().indexOf(qq[i].toLowerCase());
if(_1cc>=0){
return true;
}
}
...
function _tip(that,numberobj){
	var tab = _36f(that,numberobj[0]);
	opts = tab.panel("options");
	var tabs_tip = $(opts.tab).find("span.tabs-tip");
	var color = numberobj[2]? numberobj[2]:'red';
	if(tabs_tip.length>0){
		tabs_tip.html(numberobj[1]);
		tabs_tip.css("background-color",color);
	}else{	
		$(opts.tab).append("<span class=\"tabs-tip\" style='background:"+color+"'>"+numberobj[1]+"</span>");
	}
...
},changetip:function(jq,numberobj){
return _tip(jq[0],numberobj);
},
...
var rows=$.data(_677,"datagrid").data.group;
...
if(col.field){
td.attr("field",col.field);
td.append("<div class=\"datagrid-cell\"><span></span><span class=\"datagrid-sort-icon\"></span><span class=\"datagrid-rowcheck-icon\"></span></div>");
...
td.addClass("datagrid-header-over");
	if($(_6b6).data("datagrid").options.checkrowable){
		var cell=td.find("div.datagrid-cell");
		cell.addClass("datagrid-rowcheck");
	};
}else{
td.removeClass("datagrid-header-over");
if($(_6b6).data("datagrid").options.checkrowable){
		var cell=td.find("div.datagrid-cell");
		cell.removeClass("datagrid-rowcheck");
	};
}
}
};
...
//wying
if(opts.checkrowable){
	if($(e.target).closest(".datagrid-rowcheck-icon").length>0){
		var td=$(e.target).closest("td[field]");
		opts.onCheckRowMenu.call(_6b9,e,td.attr("field"));
	};
};
...
if($.isArray(data)){
data={total:data.length,group:data};
}
...
data.group.sort(function(r1,r2){
var r=0;
...
opts.view.onBeforeRender.call(opts.view,_72c,data.group);
}
...
if(_732.total!=data.page.total){
_731.pagination("refresh",{total:data.page.total});
...
$.data(this,"datagrid",{options:opts,panel:_7d4.panel,dc:_7d4.dc,ss:null,selectedRows:[],checkedRows:[],data:{total:0,group:[]},originalRows:[],updatedRows:[],insertedRows:[],deletedRows:[]});
}
_693(this);
_6aa(this);
_660(this);
...
return $.data(jq[0],"datagrid").data.group;
...
_856=data.group.length;
}
if(_856>data.group.length){
_856=data.group.length;
}
function _858(_859){
var _85a=_859?1:2;
for(var i=data.group.length-1;i>=_856;i--){
...
function _864(_865){
var _866=_865?1:2;
for(var i=_862+1;i<data.group.length;i++){
...
$.fn.datagrid.defaults=$.extend({},$.fn.panel.defaults,{sharedStyleSheet:false,frozenColumns:undefined,columns:undefined,fitColumns:false,resizeHandle:"right",resizeEdge:5,autoRowHeight:true,toolbar:null,striped:true,checkrowable:true,method:"post",nowrap:true,idField:null,url:null,data:null,loadMsg:"Processing, please wait ...",emptyMsg:"<span>No data</span>",rownumbers:false,singleSelect:false,ctrlSelect:false,selectOnCheck:true,checkOnSelect:true,pagination:false,pagePosition:"bottom",pageNumber:1,pageSize:20,pageList:[10,20,30,40,50],queryParams:{},sortName:null,sortOrder:"asc",multiSort:false,remoteSort:true,showHeader:true,showFooter:false,scrollOnSelect:true,scrollbarSize:18,rownumberWidth:30,editorHeight:24,headerEvents:{mouseover:_6b4(true),mouseout:_6b4(false),click:_6b8,dblclick:_6bd,contextmenu:_6c0},rowEvents:{mouseover:_6c2(true),mouseout:_6c2(false),click:_6c9,dblclick:_6d3,contextmenu:_6d7},rowStyler:function(_872,_873){
...
return $.data(_87b,"datagrid").data.group[parseInt(_87c)];
...
},onCheckRowMenu:function(e,_8a9){}});
})(jQuery);
(function($){
var _8ac;
...
_8eb={value:row[opts.groupField],rows:[row],startIndex:_8ea.data.group.length};
...
data=this.transfer(_9d0,_9d1,data.group);
}else{
...
if(opts.groupField){
var g=this.groupRows(_9f5,_9f8.data.group);
this.groups=g.groups;
_9f8.data.group=g.rows;
var _9f9=[];
...
$(_9f6).html(this.renderTable(_9f5,0,_9f8.data.group,_9f7));
}
...
function _add(node){
return node[opts.textField||""]||node.name;
};
};
